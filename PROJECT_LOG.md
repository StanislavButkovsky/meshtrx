...
### 55. Прошивка 4.4.8 — ретранслятор больше не возвращает сообщение автору — ГОТОВО ✓

**06.09.2026.** Из группы, сразу после появления третьей ноды в роли
ретранслятора: «сообщение в общий чат дублируется, видимо, репитером». Голос при
этом не двоился.

**Причина.** Ретранслятор переизлучает пакет как есть, меняя только TTL, и до
отправителя возвращается его собственное сообщение — приложение показывает его
входящим рядом с исходящим. Дедупликация по `(sender, seq)` бессильна: в её кеш
попадает принятое, а собственное мы отправляли, а не принимали.

**Исправление.** На приёме отбрасываем пакет с нашим адресом отправителя — и для
текста, и для голоса. Голос не двоился лишь потому, что во время своей передачи
рация не слушает эфир, но эхо через ретранслятор было возможно.

**Проверено** на стенде: broadcast 20/20, голос 100/100, ретранслятор пересылает
текст 5 из 5, «пакет не размножается — по одному повтору», маяки 2 из 2.

Прошивка 4.4.8, приложение остаётся 4.4.7.

### 56. Прошивка 4.4.9 — ретранслятор показывает, кого слышит — ГОТОВО ✓

**06.09.2026.** Предложение из группы: ноды и так шлют координаты, значит
ретранслятор может показать карту. Маяки он разбирал и раньше — брал адрес для
защиты от повторов, координаты выбрасывал.

**Что добавлено.** Таблица станций на 32 записи (позывной, координаты, RSSI,
заряд, время последнего маяка), выдача `/api/nodes`, страница `/map` с радаром и
картой на выбор. Радар рисуется в браузере и работает всегда; карта тянет тайлы
из интернета, поэтому грузится по нажатию и объясняет, если сети нет.

**Подключение к домашней сети со страницы**: поиск сетей, проверка пароля без
сохранения, и только потом «Подключить». Ретранслятор теперь работает
одновременно точкой доступа и клиентом (`WIFI_AP_STA`) — раньше при сохранённой
сети он уходил в чистого клиента, и если сеть не поднялась, попасть на страницу
было нечем.

**Три находки, которые дало живое устройство.** Страница висела белой полминуты:
библиотека карты грузилась из интернета, которого в режиме точки доступа нет.
Сканирование сетей блокировало веб-сервер целиком. А скрипт вовсе не выполнялся —
в двух местах кавычки внутри кавычек, и одна ошибка убивала весь код страницы.
Последнее теперь ловит `firmware/test/check_web_page.py`: собирает страницу из
литералов прошивки и проверяет через `node --check` за секунду, вместо часа с
телефоном и перепрошивками.

Прошивка 4.4.9, приложение остаётся 4.4.7.

### 57. Прошивка 4.4.10 — карта не сбрасывает масштаб — ГОТОВО ✓

**06.09.2026.** Через двадцать минут после выпуска 4.4.9 из группы: «карта
работает! только один момент, карта автоматически центруется и масштабируется по
некому таймауту — подвинул, увеличил, а она сбрасывается».

Причина прямая: страница обновляет данные каждые пять секунд, и `drawMap()` при
каждом обновлении звал `fitBounds()` по всем станциям. Теперь подгонка один раз,
при первом показе; дальше обновляются только метки. Общий охват возвращает
кнопка «Показать всех».

Прошивка 4.4.10, приложение остаётся 4.4.7.

### 58. Прошивка 4.4.11 — недопринятый голос не доигрывается шумом — ГОТОВО ✓

**07.09.2026.** Из группы: «через репитер голосовое проходит целиком, но фраза не
вся проходит, после нескольких слов — до конца записанного идёт просто шум. Без
репитера такого нет».

**Причина.** Потерянный кусок — это нули в буфере приёма, а Codec2 декодирует их
не в тишину, а в шипение. Прошивка отдавала телефону весь буфер: в коде прямо
стояло «воспроизвести в любом случае (даже битый)». Через ретранслятор потери
выше — он повторяет пакеты в том же полудуплексном канале.

**Исправление.** Голос, принятый не полностью, отдаётся по первую дыру: границу
берём из битовой карты кусков, которая и так велась для перезапросов. Если не
дошло ничего — не отдаём вовсе.

**Чего не удалось.** Воспроизвести обрезку на стенде: перезапросы дособирают файл
даже при 55 % потерь всех пакетов, а при 85 % передача не финализируется. Случай
«дошло, но с дырами» на платах в полуметре не получается — он и живёт именно на
ретрансляторе. Регрессий нет: файлы, голос, восстановление — 26 проверок из 26.

Прошивка 4.4.11, приложение остаётся 4.4.7.

### 59. Приложение 4.4.8 — крестик перестал врать про доставку — ГОТОВО ✓

**07.09.2026.** Из группы, при замерах дальности в одиночку: «не всегда корректно
определяется доставка. Пишет, что не дошло, а оно дошло. Поэтому не далеко
сегодня ушёл».

**Причина.** Приложение ждало подтверждения две секунды на попытку, делало три
попытки и рисовало крестик «не доставлено». Но оно знает лишь то, что не получило
ответа: сообщение могло дойти, а потеряться могло подтверждение — оно
возвращается тем же полудуплексным каналом, а через ретранслятор круг вдвое
длиннее. Повторы при этом уходили поверх ещё не пришедшего ответа, занимая тот
самый канал.

**Исправление.** Паузы растут: 3, 5, 8 секунд. По их исчерпании — отдельное
состояние «ушло, ответа нет» (↑); крестик остался за настоящим отказом отправки.
Позднее подтверждение ставит галочку задним числом — раньше оно игнорировалось,
потому что искало сообщения только в состоянии «ждём».

**Проверено на стенде с телефоном:** адресат уведён на другой канал — «↑» через
15 с вместо крестика; адресат на своём канале — галочка через 4 с.

Приложение 4.4.8, `versionCode` 53, подпись прежняя. Прошивка остаётся 4.4.11.

### 60. Поиск свободного канала — прошивка 4.4.12, приложение 4.4.9 — ГОТОВО ✓

**09.09.2026.** Замеры дальности показали, что голос сдаётся раньше текста, и
одной из вероятных причин был занятый канал: 868 МГц в городе делят домофоны,
метеостанции, счётчики, сигнализации. Проверить это было нечем — канал выбирался
наугад.

**Что сделано.** `loraScanChannels()` проходит по всем 23 каналам и меряет
уровень эфира (`getRSSI(false)` — шум, а не пакет). Уровень берётся как самый
громкий отсчёт серии, а не средний: помеха редко держится ровно, и по среднему
тихий на вид канал может оказаться непригодным. Команда `SCAN` в консоли стенда,
команда `0x34` по Bluetooth с ответом `0x35`, кнопка «Найти свободный канал» в
настройках приложения — три самых тихих канала с уровнями, самый шумный для
сравнения и напоминание сменить канал на всех устройствах, включая ретранслятор.

**Замер на столе:** тише всего CH 20 и CH 22 (−108 дБм), шумнее всего CH 3
(−93). Пятнадцать децибел разницы — помехи видны и измеримы.

Приём на время сканирования прерывается (радио одно), поэтому это действие по
команде человека, а не фоновая задача. Регрессий нет: текст, голос, смена
каналов — 17 проверок из 17.

Прошивка 4.4.12 и приложение 4.4.9 работают только в паре: сканирует рация.

### 61. Смена канала по эфиру — вся группа разом — ГОТОВО ✓

**10.09.2026.** Поиск свободного канала показывает, куда переходить, но
перейти нужно всем сразу: рация, оставшаяся на прежнем канале, для группы
исчезает. Обойти каждое устройство руками получается не всегда — часть раций в
этот момент в карманах у других людей, а ретранслятор может стоять на крыше.

**Что сделано.** Пакет `PKT_TYPE_CHANNEL_SET` (0xE7): новый канал и задержка
перехода. Команда идёт тремя копиями подряд — одиночный пакет в полудуплексе
теряется легко, а цена потери здесь высокая. Все, кто её услышал, включая
ретранслятор, переходят одновременно через 10 секунд; отправитель уходит
последним, пока команду ещё можно повторить.

**Возврат для отставших.** Нода, перешедшая на новый канал и не услышавшая там
никого за две минуты, сама возвращается на прежний. Чтобы «никого не слышно» не
означало «все просто молчат», после перехода каждая рация трижды отмечается
маяком с разбросом по времени. Собственная передача жизнью не считается —
для этого приём отделён от общей активности радио.

**Управление.** Кнопка «Сменить канал у всех» в настройках приложения (команда
`0x36` по Bluetooth) и кнопка `Set all` на странице ретранслятора. Команда
`CHALL <канал> [задержка]` в консоли стенда.

**Проверено на стенде:** две платы переходят с 10 на 12 и остаются вместе через
2,5 минуты; связь на новом канале работает; ушедшая в одиночку плата вернулась
сама. Ретранслятор переходит вместе с группой и остаётся на новом канале.

**Две ошибки по дороге.** Смена частоты не возобновляет приём — после
`setFrequency` нужен явный `startReceive()`, иначе рация уходит на новый канал
глухой. И номер канала в `main.cpp` живёт отдельно от радиомодуля: ретранслятор
переключался, а INFO и телефон показывали старый канал, пока эту переменную не
стали обновлять.

**Заодно исправлен стенд.** Блок проверки телефона считал, что телефон
подключится ко второй плате, а тот выбирает рацию сам. Когда он брал первую,
сообщение уходило самому себе, в эфире не появлялось ничего, и проверка
объявляла поломку связи. Теперь адресатом берётся та плата, к которой телефон
не подключён.

### 62. Смена канала: ретранслятор и рации перестали разбегаться — 4.4.14 ✓

**10.09.2026, вечер того же дня.** Из группы после выпуска 4.4.13: «на нодах
поменялся канал, на самом репитере — нет», следом — «нестабильно работает
смена каналов у всех».

**Причина одна на оба случая — возврат для отставших.** Он задуман для рации,
ушедшей на новый канал в одиночку, и меряет тишину двумя минутами. Но группа
может просто молчать: штатный маяк идёт раз в пять минут. Ретранслятор молчит
всегда — он только пересылает чужое, — поэтому уходил за группой и через две
минуты возвращался, оставляя ноды на новом канале. Воспроизведено на стенде:
команда с ретранслятора, нода уведена в сторону — откат через 2,3 минуты.

**Исправление.** Ретранслятор, разославший команду со своей страницы, не
возвращается вовсе: человек нажал кнопку осознанно. Ушедший по команде из
эфира ждёт десять минут. Столько же теперь ждёт и рация. После перехода
маяком отмечается и ретранслятор — без этого рация, ушедшая туда же,
считала, что осталась одна.

**Отвергнутая гипотеза.** Думал, команду просыпают рации без телефона: приём
урывками рассчитан на длинную преамбулу, а команда шла с обычной. Серия из
шести смен на спящей рации прошла и на прежней сборке — дело не в этом.
Длинную преамбулу оставил как страховку на дальности, но выигрыша на столе
не измерено.

**Заодно.** Страница ретранслятора показывает запланированный переход и
секунды до него: раньше она молча показывала прежний канал ещё десять секунд,
и это выглядело как несработавшая кнопка.

**Проверено:** рация, оставшаяся одна, держится через 2,5 минуты (прежняя
сборка уходила); пара «нода + ретранслятор» вместе через 2,7 минуты; три
смены подряд с обеих сторон; основные блоки 38/38.

### 63. Точка доступа ретранслятора гаснет при подключении к сети — 4.4.15 ✓

**10.09.2026, вечер.** Из группы: «на репитере нет потребности быть точкой
доступа, достаточно обычный вифи-клиент, а так, дополнительно вещает. А вифи,
всё-таки, шумит для ноды. Даже на уровне развязки платы».

Держать точку вместе с клиентом решили в записи 57 — ради доступа к странице,
если сеть не поднялась. Страховка нужная, но постоянно работающий передатчик
рядом с приёмником — цена, которую платить незачем.

**Что сделано.** Подключились к сети — точка гаснет, остаётся чистый клиент.
Сеть пропала дольше чем на полминуты — точка возвращается: полминуты выдержки
нужны, чтобы перезагрузка роутера не поднимала её каждый раз. Три минуты после
сохранения новой сети точка держится в любом случае: человек настраивает сеть,
сидя на самой точке, и вылетел бы ровно в тот момент, когда страница показывает
новый адрес.

**Мелочь, заметная в логе.** Наличие сохранённой сети теперь помним в памяти:
сторож спрашивал NVS каждые пять секунд и получал «ключа нет».

**Проверено:** без сохранённой сети точка поднимается, страница отвечает,
эфирные блоки стенда 25 из 25. Сценарий «подключился к сети — точка погасла»
на стенде не воспроизведён: нужна настоящая сеть с паролем.

**Ошибка прогона, стоившая десяти минут:** первый прогон стенда после правок
показал полное отсутствие связи. Причина была не в коде — плата осталась в
режиме ретранслятора с предыдущей ручной проверки и, разумеется, не отвечала
как обычная рация.

### 64. Клиент WiFi без сети глушил приём ретранслятора — 4.4.16 ✓

**11.09.2026.** Разделение режимов (запись 63) принесло regression: ретранслятор
без сохранённой сети стал подниматься как `WIFI_AP_STA` — точка плюс клиент,
которому некуда подключаться. Раньше в этом случае была чистая точка.

**Измерено на стенде.** С клиентом: голос через ретранслятор — 0 пересланных
пакетов из 20, один потерянный broadcast из двадцати, 20 проверок из 22. Без
клиента: 10 из 20, 20 из 20, 22 из 22. То есть включённая вхолостую клиентская
часть глушила приём — ровно то, о чём писали в группе: «вифи шумит для ноды,
даже на уровне развязки платы».

**Заодно.** В списке каналов на странице текущий канал помечен словом
«сейчас»: список показывает выбор человека, а не состояние рации, и по
присланному скриншоту это читалось как «ретранслятор путается в своём канале»
(в списке CH 16, в таблице CH 19). Проверил в браузере: выбор ничем не
затирается — ни при перезагрузке, ни при автообновлении страницы; ошибки в
прошивке не было.

**Урок.** Регрессия уехала в продакшен, потому что после правки режимов WiFi
я прогнал эфирные блоки на сборке, где сеть у ретранслятора не была сохранена,
и увидел 25 из 25 — а разница проявляется именно в блоке ретранслятора с
голосом, который в тот прогон не попал.

### 65. Вторая фраза подряд пропадала через ретранслятор — 4.4.17 ✓

**14.09.2026.** Разговор в группе о связке трёх ретрансляторов на высотках
начался с предложения «хранить идентификаторы уже переданных пакетов и не
повторять их». Механизм такой в ретрансляторе есть с самого начала — кольцевой
кеш на 64 записи, ключ «отправитель + номер + тип», окно 30 секунд. Проверка
показала, что ключ ненадёжен.

**Дефект.** Номер голосового пакета сбрасывался в ноль на каждом нажатии PTT.
Вторая фраза в пределах окна памяти ретранслятора совпадала с первой по всем
полям ключа и отбрасывалась целиком. Воспроизведено на стенде с редкими
пакетами, чтобы исключить потери полудуплекса: первая фраза 10 из 10, вторая
через пять секунд 0 из 10, отброшено все 10.

**Исправление.** Номер сквозной, начало фразы определяется по флагу
PTT_START. Память о голосе — три секунды вместо тридцати: восьмибитный номер
при двенадцати пакетах в секунду обходит круг за двадцать секунд, и в длинном
разговоре свежий пакет совпал бы со старой записью. Кеш 128 записей вместо 64.

**Почему это не решает задачу трёх ретрансляторов.** Посчитано время в эфире
для SF7 / 250 кГц / 4-7: голосовой пакет 39 байт занимает 53 мс при интервале
80 мс. Одна говорящая станция — 67% канала, с одним ретранслятором 133%, с
тремя 267%. Дедупликация убирает дубли, но не создаёт ёмкость: три
ретранслятора, слышащие друг друга, голос в одном канале не пронесут. Нужны
зоны и магистраль вне общего канала — по IP между ретрансляторами или вторым
радиомодулем. Это стоит в планах.

### 66. Главный экран: кнопка вниз, сводка наверх — 4.4.18 / приложение 4.4.11 ✓

**15.09.2026.** Из группы, со скриншотом главного экрана: «в основном, рации
заточены под левую руку, и кнопка сбоку, на телефоне такого нет. Кнопка вызова
слишком высоко, неудобно жать, можно ниже спустить, а вверху место под имя
ноды, текущий канал, состоит ли в группе, и есть ли репитер в группе. Время
последних движений с указанием RSSI и dB до получателя последнего, или, в
случае с репитером — уровень репитера».

**Что сделано.** Кнопка «Говорить» переехала вниз, под большой палец. Порядок
экрана: сводка, режимы приёма, список последних, кому говорим, статус, кнопка.
В сводке — число слышимых станций, ретранслятор с уровнем и последнее движение
в эфире с RSSI и SNR. Строка «минуту назад» стареет сама, поэтому
пересчитывается по таймеру, который снимается вместе с экраном.

**Чего не хватало в прошивке.** Ретранслятор ничем не отличался от обычной
рации: флаг BEACON_FLAG_REPEATER в протоколе был, флаги даже доезжали до
телефона в байте 27 сообщения о соседе, но прошивка их не выставляла, а
приложение не читало. Теперь выставляет и читает.

**Ошибка, которую поймал только скриншот.** Первая версия сводки дублировала
позывной и канал — они уже в шапке приложения строкой выше. Пока смотришь в
код, этого не видно: разметка фрагмента ничего не знает о шапке активности.
Установил сборку на телефон, снял экран — и дубль стал очевиден.

**Проверено:** на стенде обычная станция шлёт flags=00, ретранслятор flags=08,
эфирные блоки 23 из 23; на телефоне — «ретранслятора не слышно» сменилось на
«ретранслятор TX-C4C8 (-23 dBm)» при включении его на второй плате.

**Не сделано из просьбы:** «состоит ли в группе». Групп в приложении пока нет,
в коде заглушка с пустым списком. Вместо этого показывается число слышимых
станций.

### 67. Шифрование эфира — 4.4.19 / приложение 4.4.12 ✓

**16.09.2026.** Из группы: «жаль, что до сих пор нет приватности в переговорах
в виде шифрования». Первый этап плана из статьи «Шифрование в 39 байтах».

**Что сделано.** Шифруется нагрузка, заголовок открыт: по нему работают
ретранслятор и отбор «моё или не моё», и ключ ретранслятору не нужен — он
пересылает пакеты, не заглядывая внутрь. Шифр поточный (AES-256 CTR), длина
нагрузки не меняется; дописываются два байта эпохи и шесть байт подписи.
Голосовой пакет 39 → 47 байт, 53,4 → 60,5 мс в эфире: по округлению символов
LoRa шесть байт подписи стоят столько же, сколько четыре.

**Эпоха.** Счётчик пакетов восьмибитный и обходит круг за двадцать секунд
речи. Без эпохи поток шифровался бы одной гаммой по кругу — это раскрывает
содержимое без всякого ключа. Эпоха сдвигается на каждом обороте счётчика, а в
NVS пишется с запасом на 64 оборота: запись раз в двадцать секунд память бы не
пережила.

**Ключ из кодового слова.** Владелец спросил, нельзя ли обойтись более простым
шифром с коротким ключом. Ответ: длина ключа не влияет ни на размер пакета, ни
на эфирное время — ключ не передаётся, в эфир едут подпись и эпоха. Неудобен
был ввод 64 шестнадцатеричных символов, его и заменили кодовым словом: ключ
рация выводит сама, фраза в телефоне не хранится. Отпечаток из четырёх знаков
позволяет сверить ключ вслух.

**Два замера по дороге.** 120 000 проходов вывода ключа это 7,4 секунды —
столько человек не ждёт; взяли 50 000 (3,4 с) и честно написали в интерфейсе,
что от перебора защищает длина фразы, а не эти секунды. И длину фразы считаем в
символах: пять русских букв в байтах выглядели как восемь.

**Чего пока нет.** Файлы не шифруются: в пакете с куском файла нет адреса
отправителя, и счётчик шифра у двух говорящих может совпасть — это менять
формат пакета. Маяки тоже открыты, значит позывной и координаты видны. В
документации сказано прямо.

**Проверено:** текст и голос под общим ключом (20 из 20), без ключа и с чужим
ключом приёмник чужое не разбирает, ретранслятор без ключа пересылает как
обычно, ключ с телефона и фраза в консоли дают один отпечаток. Без ключа — 36
проверок из 36.

### 68. Флаг на карте ретранслятора — 4.4.20 ✓

**17.09.2026.** Из группы, второй раз: «уберите, плиз, флаг — ходить по городу
с антенной, которая связана с такой картой, задолбаюсь объяснять». Человек
написал повторно, потому что первая правка не работала.

**Почему не работала.** У Leaflet на флаг стоит `display:inline !important` с
тем же селектором, что добавили мы, а его стили грузятся динамически — позже
наших. При равной силе и равной специфичности выигрывает поздний.

**Исправление двумя средствами.** Селектор длиннее — специфичность выше, и
порядок загрузки больше не решает. И префикс подписи отключён вовсе: свой блок
вместо готового, в нём только источник карты.

**Проверено в браузере**, а не «по факту добавления строки»: было
«Leaflet | OpenStreetMap» с флагом, стало «OpenStreetMap».

**Про Яндекс.Карты.** Владелец спросил, нельзя ли перейти на них. Нельзя по
трём причинам: тайлы запрещено брать вне их API, ключ JS API привязывается к
домену, а у страницы ретранслятора домена нет — только адрес в локальной сети,
и наконец карта нужна там, где интернета нет вовсе. Выбор источника карты
решили не делать.

**Урок.** Правку, которая перебивает чужой стиль, проверять в браузере.

### 69. Нулевой уровень сигнала оказался верхом шкалы — 4.4.21 / 4.4.13 ✓

**17.09.2026.** «Почему RSSI иногда приходит нулевым» висело в открытых
вопросах с начала месяца. Повод разобраться дал скриншот из группы: у станции
рядом с ретранслятором стояло «0 dBm».

**Замер.** Мощность передатчика против уровня на соседней плате: −9 → −17,
−3 → −10, +3 → −4, а +8, +14, +20 и +22 дают ровно ноль. В библиотеке радио
уровень пакета считается как минус половина беззнакового числа, поэтому ноль —
верхняя граница измеримого. SNR при этом приходит нормальным, отсюда и
впечатление, что теряется именно RSSI.

**Значит бага не было.** Чинить нужно было показания: человек видит ноль и
читает «сигнала нет», хотя станция вплотную. Везде, где уровень выводится
человеку, теперь «≥0 dBm» — на странице ретранслятора, в сводке, в списке
абонентов, на радаре, в чате, в выборе адресата и на карте. В приложении это
одна функция вместо шести мест со скопированным форматированием.

**Практическое следствие для замеров.** Уровни выше −10 дБм неразличимы: при
измерениях в поле станции нельзя ставить вплотную, иначе разница мощностей в
эфире не видна вовсе.

### 70. Приложение узнаёт о выпусках, рация называет свою версию — 4.4.22 / 4.4.14 ✓

**18.09.2026.** Из группы: «добавьте, пожалуйста, функцию обновления в
приложении, чтобы не бегать каждый раз на сайт». Выпуски выходят по нескольку
раз в неделю, и человек узнавал о них, только если заходил на сайт сам.

**На сайте** появился latest.json: текущие версии и ссылки, собирается на этапе
сборки из тех же констант, что и страница загрузки, номер сборки берётся прямо
из build.gradle.

**В приложении** — строка о версиях и кнопка проверки; раз в сутки проверка
идёт сама. Без сети молчит: рация нужна там, где сети нет, и ругаться на это в
каждом запуске незачем. Установку APK Android подтверждает у человека, обойти
это нельзя, поэтому кнопка просто открывает файл.

**В прошивке** появилась версия. Её не было вовсе: понять, что стоит на рации,
можно было только вспомнив, чем её шили. Теперь она в version.h, рация называет
её по Bluetooth при подключении и по запросу, приложение показывает в строке
«Устройство» и сравнивает с сайтом.

**Защита от расхождения.** Версия записана в двух местах — в прошивке и в
константах сайта. Сборка сайта сверяет их и падает при несовпадении: проверено
сразу, подъём версии только в прошивке остановил сборку с указанием, какое
число где стоит.

**Сравнение версий** идёт по номеру сборки, а не по строке: «4.4.9» как строка
больше «4.4.13». Для прошивки, где номер только строкой, сравнение по числам
между точками; восемь случаев проверены, включая пустую строку и мусор.

**Проверено на телефоне в обе стороны:** на текущей версии «Установлена
последняя версия», на нарочно устаревшей сборке — «Вышло приложение 4.4.13, у
вас 4.4.12» и кнопка, открывающая браузер на файле. Заодно проверился случай
без сайта: файла ещё не было, и приложение честно сказало, что сайт недоступен.

### 71. Рация с ключом слушала открытый эфир — 4.4.23 ✓

**18.09.2026.** Из группы: «Heltec v3 при смене ключей шифрования (разные ключи
на разных нодах) устройства продолжают общаться будто всё по-прежнему».

**Причина моя.** Разные ключи отсекались правильно, а открытые пакеты рация с
ключом принимала — я сам это разрешил ради совместимости со старыми прошивками.
Получилось хуже, чем без шифрования: достаточно одной рации без ключа, и
разговор идёт мимо шифра, пока люди считают его закрытым. Ровно тот случай, про
который написано в нашей же статье: защита, работающая через раз, опаснее
отсутствующей, потому что на неё рассчитывают.

**Исправление.** При заданном ключе голос и текст без шифра отбрасываются.
Маяки и вызовы остаются открытыми и принимаются: иначе исчезает список
абонентов, а координаты и позывной и так не шифруются.

**Побочный эффект, о котором сказано в журнале изменений:** после обновления
рация перестанет слышать тех своих, у кого ключ не задан или прошивка старше
4.4.19. Это правильное поведение, но человек должен знать, почему связь
«пропала».

**Проверено семь сочетаний ключей**, включая маяк под ключом и работу без
ключей вовсе; полный прогон без шифрования 36 из 36.

### 72. Ключ шифрования не поднимался при старте — 4.4.24 ✓

**18.09.2026, вечер.** Из группы: «прописываю ключи, отпечатки одинаковые,
сохраняю настройки, рации не могут передать друг другу сообщения» и «после
завершения приложения ключи не сохраняются».

**Причина.** Вызов cryptoInit() при добавлении шифрования попал внутрь ветки
режима ретранслятора. Записывался ключ исправно, а поднимался при старте только
у ретранслятора — в обычном режиме рация после включения считала, что
шифрования нет.

Обе жалобы объясняются одним: пока рация не перезагружалась, всё работало и
отпечатки совпадали; после выключения одна оказывалась без ключа, и связь
пропадала — вторая, с ключом, открытый эфир уже не слушает (запись 71).

**Исправлено:** вызов перенесён до выбора режима; рация при подключении сама
сообщает телефону состояние ключа; запись ключа проверяется по числу байт.

**Урок.** Проверку «работает ли шифрование» я делал в одном сеансе, не
перезагружая платы. Всё, что хранится между включениями, надо проверять
перезагрузкой — иначе проверяется только оперативная память.

**Стенд:** связь под ключом, перезагрузка обеих, ключ на месте, текст и голос
ходят (20 из 20), снятие ключа у одной рвёт связь. Полный прогон 25 из 25.

### 73. Версия прошивки на самом устройстве — 4.4.25 ✓

**19.09.2026.** Из группы: «где можно посмотреть номер текущей версии прошивки
ноды?». Ответ был неудобный: нигде, кроме приложения, и только с 4.4.22.

Теперь версия показывается на экране рации при включении, строкой в таблице на
странице ретранслятора и в ответе INFO для стенда. Вопрос «что у меня стоит»
закрывается, не открывая телефон.

**Стенд:** INFO сообщает версию, 21 проверка из 21.

### 74. Версии приложения и прошивки — двумя строками — приложение 4.4.15 ✓

**19.09.2026.** Из группы: «отдельной строкой нужно бы вывести текущую версию
прошивки ноды, а не версию приложения, чтобы знать, актуальна ли её прошивка».

Замечание точное: версия прошивки была приписана к строке приложения и читалась
как одно целое. Теперь две строки, каждая со своим состоянием — «последняя
версия» или «вышла такая-то». Кнопка «Скачать» показывается только для
приложения: прошивку кнопкой не обновишь, она шьётся кабелем, и предлагать
обратное нечестно.

Отдельно предусмотрены честные ответы вместо пустого места: «рация не
подключена» и «не сообщает версию (старее 4.4.22)».

**Не проверено глазами.** Установка на стендовый телефон требует подтверждения
на самом устройстве, а хозяина рядом не было; проверено, что сборка проходит и
идентификаторы разметки совпадают. Экран посмотреть при первой возможности.

### 75. Видно, когда ключи разошлись — 4.4.26 / 4.4.16 ✓

**19.09.2026.** Из группы: «при применении ключей ноды перестают получать
сообщения друг друга, без ключей всё работает». У человека два разных телефона,
Android 15 и LineageOS, и две рации.

Самая частая причина такого — разошедшиеся фразы: клавиатура добавляет пробел
в конце или подменяет букву, ключ получается другой, и рации честно перестают
слышать друг друга. Снаружи это неотличимо от тишины.

**Что сделано.** Рация сообщает телефону, что слышит пакеты, но разобрать их
своим ключом не может; приложение показывает это строкой под отпечатком. То же
сообщение идёт, когда рядом говорят без шифрования, а у нас ключ задан.
Уведомление не чаще раза в три секунды, чтобы не залить канал при потоке
голоса.

**Вывод для интерфейса.** Там, где защита что-то отбрасывает, человек должен
видеть причину. Молчание после включения шифрования выглядит как поломка, и
первое, что делает человек, — выключает шифрование.

**Стенд:** при разных ключах «пакет не расшифрован: чужой ключ», при
одинаковых связь работает; прогон 17 из 17.

### 76. Текст с телефона уходил мимо шифрования — 4.4.27 / 4.4.17 ✓

**19.09.2026.** Из группы: «отпечатки совпадают, рации с ключом не принимают
сообщения». Отпечатки и правда совпадали — причина была не в ключах.

**У текста два пути отправки.** Через общую очередь (так шлёт стенд) и прямой
(так уходят сообщения из приложения). Зашифровал я только первый. Рация с
ключом законно отбрасывала открытые пакеты, и выглядело это как «шифрование
ломает связь». Тем же прямым путём уходил будящий пакет голоса — первый кадр
фразы.

**Почему не поймал сам.** Все мои проверки шли консолью стенда, то есть по
зашифрованному пути. Проверка «текст под ключом доходит» была зелёной, а
человек с телефоном видел обратное. Урок: проверять надо тем же путём, которым
пользуется человек, а не тем, который удобен стенду. Теперь в стенде есть
проверка отправки с телефона под ключом.

**Заодно:** приложение не видело ключ после перезапуска — рация сообщала
состояние раньше, чем телефон подписывался на уведомления. Теперь через
полторы секунды, плюс приложение спрашивает само.

**И ещё:** стенд через раз считал, что режим ретранслятора не включился.
Команда печатала ответ и сразу перезагружала плату, обрывая передачу. Теперь
ответ дожидается отправки — гонка, которая портила прогоны неделями.

**Стенд:** 25 из 25, отправка с телефона под ключом доходит.

### 77. Слышать открытый эфир — по выбору человека — 4.4.28 / 4.4.18 ✓

**19.09.2026.** Из группы: «связь с ключами работает. Есть неприятный момент:
если на одной ноде включено шифрование, она перестаёт видеть общий
незашифрованный чат».

Поведение было задумано намеренно (запись 71): рация с ключом не слушает
открытое, иначе одна рация без ключа сводит шифрование на нет. Но выбор должен
принадлежать человеку: кому-то нужен и общий чат, и свои под ключом.

**Сделано:** переключатель в настройках, по умолчанию выключен. Когда включён,
открытые сообщения видны и помечаются замком — защищённое и незащищённое не
должны выглядеть одинаково. Рация сообщает телефону отдельным байтом, было ли
сообщение зашифровано; старые прошивки его не шлют, и тогда пометки нет.

Настройка хранится в рации, а не в телефоне: она о том, что рация принимает из
эфира.

**Стенд:** по умолчанию открытое отбрасывается, после включения принимается,
шифрованное принимается всегда; прогон 17 из 17.

### 78. Очистка истории чата — приложение 4.4.19 ✓

**20.09.2026.** Из группы: «не хватает кнопки очистить историю чата».

Кнопка поставлена в самом чате, в строке фильтра, а не в настройках: её ищут
там, где читают переписку. Подтверждение обязательно и называет число
сообщений — удалённое не вернуть.

В тексте подтверждения прямо сказано, что переписка хранится только на этом
телефоне и у собеседников копии останутся: человек должен понимать, что
чистит у себя, а не у всех.

**Не проверено глазами.** На стендовом телефоне висит системное предупреждение
об установке, которое должен нажать человек; версия там осталась 4.4.13. Сборка
проходит, разметка и обработчик на месте.

### 79. Версия прошивки не доходила до телефона — 4.4.29 / 4.4.20 ✓

**21.09.2026.** Экран стендового телефона снова стал доступен, и первое же
включение показало то, чего не видно в коде: приложение писало «прошивка не
сообщает версию (старее 4.4.22)», хотя на рации стояла 4.4.28.

**Причина.** Рация отправляла версию и состояние ключа двумя уведомлениями
подряд. Стек BLE второе принимал, первое терялось — до телефона доходило
только состояние ключа. Разнёс отправку по времени: версия через полторы
секунды после подключения, ключ через две с небольшим. В журнале приложения
появилось «Прошивка рации: 4.4.28».

**Заодно** убран дубль: строка «Устройство» повторяла версии приложения и
прошивки, которые накануне вынесены отдельными строками.

**Проверено глазами** — то, ради чего и стоило дождаться телефона: кнопка
очистки чата на месте, подтверждение называет точное число («Удалено будет
125»), после очистки история пуста и остаётся пустой после перезапуска.
Раздел шифрования и переключатель «слышать открытый эфир» на месте.

**Отдельная находка.** Установки APK срывались не из-за человека: на телефоне
стояла отладочная сборка, а ставилась подписанная — Android отказывает при
разных подписях. Выглядело это как «подтверждение не нажали».

### 80. «Сменить канал у всех» и пометка «без шифра» — 4.4.30 / 4.4.21 ✓

**21.09.2026.** Две жалобы из группы, обе про то, что кнопка «делает вид».

**Кнопка «сменить канал у всех» молчала.** В приложении выбор канала сразу
переводит свою рацию, и человек жмёт «сменить у всех», уже стоя на нужном
канале. А прошивка в этот момент выходила из `broadcastChannelSet` по условию
`newChannel == currentChannel` — команда в эфир не уходила вовсе. Теперь совпадение
канала не отменяет рассылку: уходит команда, не планируется только собственный
переход (`alreadyHere`). То же исправлено в ретрансляторе. В приложении кнопка
открывает отдельный список каналов с пометкой «(сейчас)», а не переиспользует
текущий выбор.

**Замок у открытых сообщений читался наоборот.** 🔓 рядом с сообщением половина
понимала как «защищено». Заменено словами: «без шифра». Проверено на живом
телефоне — открытое сообщение приходит со строкой «11:16 −29dBm без шифра»,
зашифрованное с той же парой раций — просто «11:18 −29dBm».

**Ложная тревога.** Перед этим казалось, что открытое сообщение рация принимает,
а приложение его не показывает. Оказалось, проверочный текст слался на адрес
`FFFF`, а широковещательный адрес в протоколе — `0000`: пакет честно
отбрасывался как чужой. Ошибка была в проверке, а не в прошивке.

**Стенд:** 17/17 (ping, текст, голос 100/100, каналы, стабильность).

### 81. Бот убран: группу читаем от своего имени — 22.09.2026 ✓

**Как было.** С 15.09.2026 переписку группы читал собственный бот: на сервере
сайта круглосуточно работал `meshtrx-tgbot` — длинный опрос Telegram, SQLite с
перепиской и MCP-сервер, который Claude Code поднимал по ssh. Всё держалось на
одном ограничении Bot API: `getUpdates` хранит непрочитанное около суток,
поэтому демон обязан был работать всегда, а значит жить на сервере.

**Почему ушли.** В соседнем проекте та же задача решена без сервера:
пользовательский клиент MTProto (Telethon) от личного аккаунта. История группы
доступна за любой срок, поэтому непрерывно работающий процесс не нужен вовсе —
лента поднимается на время сессии и гаснет вместе с ней. Заодно отпадают три
вещи, каждая из которых уже отнимала время: токен бота на чужой машине,
подбор живого адреса `api.telegram.org` из России и молчаливая потеря
сообщений, когда два процесса делят один длинный опрос.

**Что стало.** Инструмент `~/.meshtrx-tg/tg-watch.py` — вне репозитория:
файл сессии это полный доступ к аккаунту, а репозиторий публичный. Чтение —
`pull`, лента в сессии — `watch-feed.sh` раз в 90 секунд, отправка — `send`
с текстом из файла, каждое отправленное пишется в `sent.jsonl`.

**Что потеряли осознанно.** Команда `/ask` и поиск ботом по документации. Люди
задают вопросы обычным сообщением, документация читается прямо из репозитория —
а отвечал на `/ask` всё равно не бот, а агент.

**Сообщения теперь уходят от имени владельца,** а не от бота. Это и было целью:
в маленькой группе разговор с живым человеком честнее, чем с ботом, за которым
стоит тот же человек. Цена — ошибка в тексте выглядит как его ошибка, поэтому
текст берётся из файла, а не из строки оболочки, и всё отправленное журналится.

**Сервер вычищен целиком:** сервис, юнит, `/opt/meshtrx` (717 МБ), пользователь
`meshtrx` и запись маршрута в `/etc/hosts`. Сайт на той же машине не затронут.

### 82. Уведомления о сообщениях — 4.4.31 / 4.4.22 ✓

**22.09.2026.** Просьба из группы: «что насчёт системных уведомлений о новых
сообщениях из личных чатов или даже всех». Случай очевидный — рация ловит
сообщение, когда телефон в кармане, и узнать об этом было нельзя.

**Чего не хватало в прошивке.** Телефон не мог отличить личное сообщение от
общего: в BLE-пакете `RECV_MESSAGE` никогда не передавался адрес получателя —
только текст, отправитель и RSSI. Рация проверяла адрес сама и молча решала,
показывать ли пакет, а телефону результат проверки не сообщала. Добавлен
признак «адресовано нам» последним байтом; заодно исправлен буфер, которому
при тексте в 85 символов не хватало одного байта.

**Как сделано в приложении.** Два канала уведомлений: личные — с высокой
важностью и звуком, общий чат — обычной и тихо. Разделение именно каналами, а
не полями уведомления: в Android важность задаётся каналу, и человек может
потом настроить каждый по-своему средствами системы.

Уведомление не показывается, пока приложение открыто: сообщать о том, что и так
на экране, — шум. Для этого в состояние добавлен признак видимости, который
выставляет главный экран.

Номер уведомления свой у каждого отправителя и отдельно у личного и общего:
иначе тихое сообщение в общий чат затирало личное от того же человека, что и
обнаружилось на первой проверке — второе уведомление просто заменило первое.

**Настройка** — «обо всех» (по умолчанию), «только о личных», «не уведомлять».
Телефонная, на рацию не уходит, поэтому сохраняется сразу, без «Применить».

**Проверено на живом телефоне,** все три режима: при открытом приложении
уведомлений нет; в свёрнутом приходят два разных — «TX-C4C8 — лично вам» со
звуком и «TX-C4C8 — в общий чат» тихо; в режиме «только о личных» общее
молчит; в «не уведомлять» молчат оба. Стенд 17/17.
