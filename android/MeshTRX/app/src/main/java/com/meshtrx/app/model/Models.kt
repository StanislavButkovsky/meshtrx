...
/** Уровень сигнала для человека.
 *
 *  Ноль дБм — не «нет сигнала», а верх шкалы: приёмник считает уровень как
 *  минус половину беззнакового числа, поэтому выше нуля значение не поднимается
 *  и упирается в него, когда рации стоят рядом. Показывать при этом голый ноль
 *  значит вводить в заблуждение ровно наоборот. */
fun formatRssi(rssi: Int): String = if (rssi >= 0) "≥0dBm" else "${rssi}dBm"
