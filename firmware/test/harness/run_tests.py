...
def calibrate_link(A: Device, B: Device) -> int:
    ...
    for attempt in range(5):
        ...
        time.sleep(0.5)
    # Сигнал сильнее рабочего окна — это не отказ связи, а слишком близкие
    # платы: на минимальной мощности сделать тише уже нечем. Такой прогон
    # засчитываем, но говорим вслух — при перегруженном приёмнике часть
    # проверок может вести себя странно, и знать причину полезнее, чем видеть
    # красную строку там, где связь как раз в порядке.
    if len(rssi) == 4 and avg > target_hi:
        check("канал в рабочем окне", True,
              f"{avg} дБм при {dbm:+d} дБм — сильнее окна, платы стоят близко")
        return dbm
