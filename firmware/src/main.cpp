...
// рации в поле.
// Сколько ждать голосов на новом канале, прежде чем счесть, что ушёл один.
// Две минуты оказались слишком коротким сроком: группа может просто молчать,
// а штатный маяк идёт раз в пять минут — рации по краям сети успевали решить,
// что остались одни, и уходили обратно, разрывая только что собранную группу.
#define CHANNEL_PROBE_MS   600000
...
static void broadcastChannelSet(uint8_t newChannel, uint8_t delaySec = 10) {
  ...
  // Длинная преамбула обязательна: рация без телефона слушает эфир урывками и
  // просыпается только на неё. С обычной преамбулой команду слышали лишь те,
  // с кем в этот момент был разговор, — со стороны это выглядело как «работает
  // через раз». Команда редкая, лишние миллисекунды в эфире того стоят.
