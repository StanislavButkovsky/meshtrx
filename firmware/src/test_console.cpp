...
static void handleLine(char* line) {
  ...
  if (strcmp(cmd, "FEM") == 0) {
    ...
    evt("EVT FEM rev=%d power=%d en=%d ctx=%d\n", BOARD_V4_REV,
      digitalRead(PA_FEM_POWER), digitalRead(PA_FEM_EN),
      digitalRead(PA_FEM_CTX));
#else
  ...
  if (strcmp(cmd, "REPEATER") == 0) {
    ...
    if (arg && (strcasecmp(arg, "ON") == 0 || strcasecmp(arg, "OFF") == 0)) {
      ...
      // Дождаться, пока строка действительно уйдёт в порт: перезагрузка рвёт
      // передачу на полуслове, и стенд считал, что команда не выполнена, хотя
      // режим переключился. Гонка выигрывалась через раз.
      Serial.flush();
      delay(300);
      ESP.restart();
      return;
