...
// Записей столько, чтобы кеш не вытеснялся быстрее, чем истекает срок жизни
// записи: голос идёт 12 пакетов в секунду, и при 64 записях трёх говорящих
// хватало, чтобы забыть начало собственной секунды.
#define DEDUP_CACHE_SIZE  128
#define DEDUP_LIFETIME_MS 30000  // текст, файлы, вызовы — 30 секунд
// Голосу столько помнить нельзя. Номер голосового пакета восьмибитный и при
// 12 пакетах в секунду обходит круг за двадцать секунд — короче тридцати, так
// что в длинном разговоре свежий пакет совпал бы со старой записью и пропал.
// В сети пакет живёт доли секунды, и трёх секунд хватает, чтобы отсечь эхо.
#define DEDUP_AUDIO_MS    3000
