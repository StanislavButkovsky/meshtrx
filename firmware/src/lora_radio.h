...
//
// У обеих микросхем управляющая линия одна и работает одинаково: HIGH — сигнал
// идёт через усилитель мощности на передачу, LOW — через малошумящий усилитель
// на приём. Отличается только вывод: GPIO46 у 4.2 (CPS) и GPIO5 у 4.3 (CTX).
//
// У 4.3 эта линия долгое время считалась «режимом приёма» и никогда не
// поднималась: передача уходила в антенну по приёмному тракту, мимо усилителя.
// Замер на паре плат при одинаковых 14 дБм: V3 без усилителя приходила на -56
// дБм, V4.3 с усилителем — на -72. Шестнадцать децибел потери там, где должен
// быть выигрыш, — отсюда и жалобы «на десяти метрах связи нет».
//
// GPIO46 у ESP32-S3 ещё и strapping-пин, поэтому вне передачи держим его LOW —
// это совпадает с тем, что нужно для приёма, так что правило одно на обе платы.
#ifdef BOARD_V4
  #define PA_FEM_POWER 7   // VFEM_Ctrl — питание усилителя (HIGH = вкл)
  #define PA_FEM_EN    2   // CSD — включение микросхемы (HIGH = вкл)
  #if BOARD_V4_REV >= 43
    #define PA_FEM_CTX 5    // KCT8103L CTX
  #else
    #define PA_FEM_CTX 46   // GC1109 CPS
  #endif
#endif
