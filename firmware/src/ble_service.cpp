...
void bleInit() {
  ...
  // Объявления о себе в эфире.
  //
  // Имя обязано быть в основном пакете объявления. В нём всего 31 байт, из
  ...
  pAdvertising->setMinInterval(160);        // 100 мс — интервал объявлений
...
bool bleEnsureAdvertising() {
  // Устройство, которое не объявляет о себе и ни с кем не соединено, для
  ...
  // такое соединение висит, устройство не объявляет о себе, и для остальных
