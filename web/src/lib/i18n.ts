...
export const translations = {
  ...
  'hero.desc': { ru: 'Голос, сообщения и файлы без интернета и сотовой сети. Heltec V3 или V4, телефон на Android, разговор под общим ключом группы.', en: 'Voice, messages and files with no internet and no cellular. Heltec V3 or V4, an Android phone, and conversations closed by a group key.' },
  'hero.download': { ru: 'Скачать APK', en: 'Download APK' },
  'hero.flash': { ru: 'Прошить модуль', en: 'Flash Firmware' },

  // Stats
  'stats.range': { ru: 'Голос в поле', en: 'Voice in the field' },
  'stats.range.hint': { ru: 'Текст дальше; 5+ км — прямая видимость', en: 'Text goes further; 5+ km needs line of sight' },
  ...
  // Roadmap
  'feature.crypto.title': { ru: 'Шифрование эфира', en: 'Encrypted traffic' },
  'feature.crypto.desc': { ru: 'Кодовое слово группы закрывает голос и текст. Ключ выводит сама рация, отпечаток из четырёх знаков сверяют вслух. Ретранслятору ключ не нужен: он пересылает разговор, не слыша его.', en: 'A group passphrase closes voice and text. The radio derives the key itself, and a four-character fingerprint is checked out loud. The repeater needs no key: it forwards the conversation without hearing it.' },
  'feature.channels.title': { ru: 'Работа с каналами', en: 'Channel handling' },
  'feature.channels.desc': { ru: 'Рация проходит по всем 23 каналам и показывает, где тише всего. Выбранный канал одной командой уходит всей группе — включая ретранслятор, обходить устройства руками не нужно.', en: 'The radio walks all 23 channels and shows where it is quietest. One command moves the whole group to the chosen channel, repeater included — no need to walk up to each device.' },
  ...
  'roadmap.encrypt.title': { ru: 'Шифрование файлов и маяков', en: 'Encrypting files and beacons' },
  'roadmap.encrypt.desc': { ru: 'Голос и текст уже закрыты кодовым словом группы. Файлы и маяки пока идут открытыми, то есть позывной и координаты видны постороннему: у пакета с куском файла нет адреса отправителя, и это требует поменять формат, а не обернуть существующий.', en: 'Voice and text are already closed by a group passphrase. Files and beacons still travel in the clear, so call signs and coordinates are visible: a file-chunk packet carries no sender address, which means changing the format rather than wrapping the existing one.' },
  'roadmap.zones.title': { ru: 'Зоны и магистраль', en: 'Zones and a trunk link' },
  'roadmap.zones.desc': { ru: 'Три ретранслятора, слышащие друг друга, голос в одном канале не пронесут: одна говорящая станция занимает две трети эфира. Нужны зоны, где ретранслятор повторяет только своих, и отдельная линия между ретрансляторами.', en: 'Three repeaters that hear each other cannot carry voice in one channel: a single talking station takes two thirds of the air. That needs zones, where a repeater only repeats its own stations, and a separate link between repeaters.' },
  'roadmap.longrange.title': { ru: 'Дальний режим для текста', en: 'Long-range mode for text' },
  'roadmap.longrange.desc': { ru: 'Голос требует скорости, скорость отнимает дальность. Отдельная медленная передача для текста добьёт туда, где голос уже не живёт — замеры в поле показали, что разрыв между ними заметный.', en: 'Voice needs speed, and speed costs range. A separate slow mode for text will reach where voice no longer lives — field measurements showed the gap between them is wide.' },
  ...
  // iOS download page
  'ios.title': { ru: 'MeshTRX для iPhone', en: 'MeshTRX for iPhone' },
  'ios.subtitle': { ru: 'Установка через AltStore — без App Store и без платного аккаунта Apple', en: 'Install via AltStore — no App Store, no paid Apple account' },
  'ios.btn': { ru: 'Скачать IPA', en: 'Download IPA' },
  'ios.badge': { ru: 'Бета-версия для тестировщиков', en: 'Beta for testers' },
  'ios.what.title': { ru: 'Что такое AltStore', en: 'What is AltStore' },
  'ios.what.desc': { ru: 'AltStore — это программа, которая позволяет устанавливать приложения на iPhone без App Store. Она подписывает IPA-файл вашим бесплатным Apple ID и ставит приложение на телефон через компьютер.', en: 'AltStore is a tool that lets you install apps on iPhone without the App Store. It signs the IPA file with your free Apple ID and installs the app on your phone via a computer.' },
  'ios.limits.title': { ru: 'Ограничения', en: 'Limitations' },
  'ios.limits.1': { ru: 'Подпись действует 7 дней — AltStore переподписывает автоматически, если iPhone и компьютер в одной Wi-Fi сети', en: 'Signature lasts 7 days — AltStore re-signs automatically when iPhone and computer are on the same Wi-Fi' },
  'ios.limits.2': { ru: 'Максимум 3 приложения через AltStore одновременно', en: 'Maximum 3 apps via AltStore at a time' },
  'ios.limits.3': { ru: 'Нужен компьютер (Mac или Windows) для первой установки и переподписи', en: 'A computer (Mac or Windows) is needed for first install and re-signing' },
  'ios.steps.title': { ru: 'Инструкция по установке', en: 'Installation guide' },
  'ios.step1.title': { ru: 'Установите AltServer на компьютер', en: 'Install AltServer on your computer' },
  'ios.step1.desc': { ru: 'Скачайте AltServer с официального сайта и установите на Mac или Windows.', en: 'Download AltServer from the official website and install it on Mac or Windows.' },
  'ios.step2.title': { ru: 'Установите AltStore на iPhone', en: 'Install AltStore on iPhone' },
  'ios.step2.desc': { ru: 'Подключите iPhone к компьютеру по USB. В меню AltServer выберите «Install AltStore» → ваш телефон. Войдите со своим Apple ID.', en: 'Connect iPhone to your computer via USB. In AltServer menu select "Install AltStore" → your phone. Sign in with your Apple ID.' },
  'ios.step3.title': { ru: 'Доверьтесь разработчику', en: 'Trust the developer' },
  'ios.step3.desc': { ru: 'На iPhone откройте Настройки → Основные → VPN и управление устройством → ваш Apple ID → «Доверять».', en: 'On iPhone open Settings → General → VPN & Device Management → your Apple ID → "Trust".' },
  'ios.step4.title': { ru: 'Скачайте IPA-файл MeshTRX', en: 'Download MeshTRX IPA file' },
  'ios.step4.desc': { ru: 'Скачайте файл на iPhone через Safari, нажав кнопку «Скачать IPA» выше.', en: 'Download the file on iPhone via Safari by clicking "Download IPA" above.' },
  'ios.step5.title': { ru: 'Установите через AltStore', en: 'Install via AltStore' },
  'ios.step5.desc': { ru: 'Откройте скачанный файл и выберите «Открыть в AltStore». Или: откройте AltStore → My Apps → «+» → выберите файл.', en: 'Open the downloaded file and choose "Open in AltStore". Or: open AltStore → My Apps → "+" → select the file.' },
  'ios.step6.title': { ru: 'Подключите модуль', en: 'Connect the module' },
  'ios.step6.desc': { ru: 'Включите Bluetooth на iPhone, откройте MeshTRX и подключитесь к LoRa-устройству.', en: 'Enable Bluetooth on iPhone, open MeshTRX and connect to the LoRa device.' },
  'ios.resign.title': { ru: 'Переподпись каждые 7 дней', en: 'Re-signing every 7 days' },
  'ios.resign.desc': { ru: 'Бесплатный Apple ID подписывает приложение на 7 дней. После этого MeshTRX перестанет открываться. AltStore переподпишет автоматически, если:', en: 'A free Apple ID signs the app for 7 days. After that MeshTRX will stop opening. AltStore re-signs automatically if:' },
  'ios.resign.1': { ru: 'AltServer запущен на компьютере', en: 'AltServer is running on your computer' },
  'ios.resign.2': { ru: 'iPhone и компьютер в одной Wi-Fi сети', en: 'iPhone and computer are on the same Wi-Fi network' },
  'ios.resign.3': { ru: 'AltStore периодически открывается на iPhone (достаточно фоновой работы)', en: 'AltStore opens periodically on iPhone (background activity is enough)' },
  'ios.resign.manual': { ru: 'Если пропустили — подключите iPhone к компьютеру по USB, откройте AltStore и нажмите «Refresh».', en: 'If you missed the window — connect iPhone to computer via USB, open AltStore and tap "Refresh".' },
  'ios.alt.title': { ru: 'Альтернативы AltStore', en: 'Alternatives to AltStore' },
  'ios.alt.sideloadly': { ru: 'Sideloadly — аналог с более простым интерфейсом', en: 'Sideloadly — similar tool with a simpler interface' },
  'ios.alt.trollstore': { ru: 'TrollStore — перманентная установка без переподписи (только определённые версии iOS)', en: 'TrollStore — permanent install without re-signing (specific iOS versions only)' },
